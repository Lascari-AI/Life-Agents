# Package Type-Specific Instructions

The orchestrator selects the appropriate block based on section name and passes it to the `tax-package` subagent.
...
Write both files. The estimated_taxes output goes to the output path. The retirement output goes to the same directory with filename `retirement.json`.
